...
#Тест-кейсы значения 1050
data_tests = [1050, 1050, 1050]
running_tests(data_tests)


def test_co2_1050():
    global count_tests
    co2_element.screenshot('./output/19.png')
    assert 1 == 1


def test_water_1050():
    global count_tests
    water_element.screenshot('./output/20.png')
    assert 1 == 1


def test_energy_1050():
    global count_tests
    energy_element.screenshot('./output/21.png')
    assert 1 == 1
