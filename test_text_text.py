    ...
    def interceptor(request):
        if request.url == 'https://www.avito.ru/web/1/charity/ecoImpact/init':
            request.create_response(
                ...
                body='{"result": {"blocks": {"personalImpact": {"avatarUrl": "https://30.img.avito.st/image/1/1.Ur5lira26FdTLXxRTasfs8kp_l3bqfjV1yn8.GfM3Tvdu-ZqXKZ14aDfya5FCvBE86e6Pm7wTD9V_q2w","data": {"co2": "' + co2 + '","energy": "' + energy + '","materials": 0,"pineYears": 0,"water": "' + water + '"}}},"isAuthorized": true}, "status": "ok"}'
            )
...
data_tests = ["hello", "Murka", "LOL"]
running_tests(data_tests)


def test_co2_text_text():
    global count_tests
    co2_element.screenshot('./output/31.png')
    assert 1 == 1


def test_water_text_text():
    global count_tests
    water_element.screenshot('./output/32.png')
    assert 1 == 1


def test_energy_text_text():
    global count_tests
    energy_element.screenshot('./output/33.png')
    assert 1 == 1
